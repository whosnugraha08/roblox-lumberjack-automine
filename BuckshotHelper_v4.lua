-- =============================================
-- BUCKSHOT HELPER v4
-- ULTIMATE: Strict Table + Direct Module Hooks
-- =============================================
...
local ClientModule, VisualsHandler, TurnHandler, GameHandler
pcall(function()
...
-- Main Frame (Visual Excellence: Sleek Dark HSL theme)
...
TitleText.Text = "🔫 BUCKSHOT HELPER v4"
...
local function detectTable()
    ...
    if ok and data and data.tableId then
        ...
        -- Call updateShells to keep advice and probability values in sync
        updateShells()
...
-- =============================================
-- ROBUST MODULE HOOKS (Intercepts client events)
-- =============================================
_G.BH_Hooks = _G.BH_Hooks or {}

local function hook(mod, modName, funcName, newFunc)
    if not mod then return end
    _G.BH_Hooks[modName] = _G.BH_Hooks[modName] or {}
    local orig = _G.BH_Hooks[modName][funcName] or mod[funcName]
    _G.BH_Hooks[modName][funcName] = orig
    mod[funcName] = function(...)
        return newFunc(orig, ...)
    end
end

-- 1) VisualsHandler:DisplayNewShells (Loads shells at start of round)
hook(VisualsHandler, "VisualsHandler", "DisplayNewShells", function(orig, self, data, ...)
    pcall(function()
        if data and data.tableId then
            ...
            if STATE.tableId and data.tableId == STATE.tableId then
                if data.action == "revealShells" then
                    ...
                    STATE.round = 1 -- Match is active!
                    STATE.history = {} -- Clear old round history
                    lHist.Text = "—"
                    updateShells()
                end
            end
...
-- 2) VisualsHandler:ShootShotgun (Tracks bullet subtraction)
hook(VisualsHandler, "VisualsHandler", "ShootShotgun", function(orig, self, tblId, targetPlayer, isLive, isDead, ...)
    pcall(function()
        if STATE.tableId and tblId == STATE.tableId then
            ...
            if isLive then
                ...
                addShot("live", tName)
                -- Flash GUI border red as premium feedback
    ...
    return orig(self, tblId, targetPlayer, isLive, isDead, ...)
end)

-- 3) TurnHandler:StartTurn
hook(TurnHandler, "TurnHandler", "StartTurn", function(orig, self, ...)
    pcall(function()
        lTurn.Text = "🟢 YOUR TURN!"
        lTurn.TextColor3 = Color3.fromRGB(0, 255, 100)
        -- Premium micro-animation advice flash
...
-- 4) TurnHandler:EndTurn
hook(TurnHandler, "TurnHandler", "EndTurn", function(orig, self, ...)
    pcall(function()
...
-- 5) TurnHandler:SetTurnText (Syncs turn changes)
hook(TurnHandler, "TurnHandler", "SetTurnText", function(orig, self, enabled, player, ...)
    pcall(function()
        if enabled then
            if player == LP then
                lTurn.Text = "🟢 YOUR TURN!"
    ...
    return orig(self, enabled, player, ...)
end)

-- 6) GameHandler:StartGame (Match loading)
hook(GameHandler, "GameHandler", "StartGame", function(orig, self, ...)
    pcall(function()
        STATE.history = {}
        STATE.live = 0
        STATE.blank = 0
        STATE.round = 1 -- Started
        lHist.Text = "—"
        detectTable()
        updateShells()
    end)
...
-- 7) GameHandler:EndGame (Match finished)
hook(GameHandler, "GameHandler", "EndGame", function(orig, self, ...)
    pcall(function()
        STATE.tableId = nil
        STATE.spotIndex = nil
        STATE.maxPlayers = nil
...
-- =============================================
-- FALLBACK REMOTE EVENT & POLLING LOOP
-- =============================================
local remotes = game:GetService("ReplicatedStorage"):FindFirstChild("Remotes")

-- ShellTrackerAction Fallback
...
-- General Polling Loop: Updates every 0.5s
task.spawn(function()
    while SG.Parent do
        detectTable()
        
        -- STRICT POLLING: Only read ShellTrackerUI if built-in tracker is ACTUALLY VISIBLE
        if STATE.tableId and STATE.round > 0 then
            pcall(function()
                local ui = PG:FindFirstChild("ShellTrackerUI")
                if ui and ui.Enabled == true then -- Prevent reading stale template values
...
local hookCount = 0
for _ in pairs(_G.BH_Hooks) do hookCount += 1 end
print("[BH] v4 loaded | Hooks: " .. hookCount .. " modules | Active Table: " .. tostring(STATE.tableId))
